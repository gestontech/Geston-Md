import { createCanvas, loadImage, GlobalFonts } from '@napi-rs/canvas';
  ...
  async function generateGroupStatsCanvas({ groupName, members, totalMsg, activeUsers, topYappers, activityData, botName, timestamp }) {
      ...
      cards.forEach((card, i) => {
          ...
          ctx.fillText(`↑ ${moment(timestamp).tz('Africa/Kinshasa').format('HH:mm')} EAT`, cx + 18, cy + 108);
      });
      curY += CARDS_H + GAP;
      ...
      ctx.fillText(
          `${botName}  ·  ${moment(timestamp).tz('Africa/Kinshasa').format('DD MMM YYYY, HH:mm')} EAT  ·  Geston-MD Auto Report`,
          W / 2, curY + 28
      );
  ...
  export async function generateAndSendReport(client, targetGroupJid) {
      try {
          ...
          const caption = `*[ 📊 GESTON-MD DAILY REPORT ]*\n\nGroup: ${groupName}\nMembers: ${members}\nTotal Messages: ${totalMsg.toLocaleString('en-US')}\nActive Users: ${activeUsers}\n\nPowered by Geston-MD Auto Report`;
          ...
          const img = await generateGroupStatsCanvas({
              ...
              topYappers,
              activityData,
              botName: global.botname || client.user?.name || 'Geston-MD',
              timestamp: Date.now()
          });
