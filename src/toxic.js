...
try {
    _toxicPict = fs.readFileSync(path.resolve(__dirname, '../Geston.jpg'));
} catch (e) {
        ...
        (async () => {
            if (m.message?.protocolMessage?.type === 0) {
                ...
                if (deletedMessage) {
                        ...
                        const deleteTime = new Date().toLocaleString('en-US', { timeZone: 'Africa/Kinshasa' });
                    ...
                    try {
                        ...
                        if (editKey && newMessage) {
                            ...
                            if (editedRemoteJid !== 'status@broadcast' && !editedRemoteJid.includes('@broadcast') && !editedRemoteJid.includes('@newsletter')) {
                                ...
                                const editTime = new Date().toLocaleString('en-US', { timeZone: 'Africa/Kinshasa' });
                                let originalText = '';
        ...
        if (cmd) {
            try {
                ...
                const cmdCtx = {
                    ...
                    itsMe, isDev, isSudo, isEnvOwner, Owner, settings, Tag, msgToxic, budy, sock: client, conn: client, wa: client, bot: client, geston: client, store,
