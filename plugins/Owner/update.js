...
const REPO = (process.env.GITHUB_REPO || 'xhclintohn/Geston-MD').replace(/^https?:\/\/github\.com\//, '').replace(/\.git$/, '');
const SKIP = new Set(['.git','node_modules','.env','config.env','session.json','creds.json','auth','Session','session']);
const doRestart = (root) => { setTimeout(() => exec('pm2 restart geston-md || pm2 restart all', { cwd: root }, () => process.exit(0)), 1500); };
...
async function performZipUpdate(client, m, opts, root) {
    ...
    const exRoot  = path.join(root, '.geston_update');
