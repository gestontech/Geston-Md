...
const GITHUB_REPO = (process.env.GITHUB_REPO || 'xhclintohn/Geston-MD').replace(/^https?:\/\/github\.com\//, '').replace(/\.git$/, '');
const EXCLUDE_TOP = new Set(['.git', 'node_modules', '.env', 'config.env', 'session.json', 'creds.json', 'whatsasena.json', 'auth', 'Session', 'session']);

const restart = (root) => {
    setTimeout(() => {
        exec('pm2 restart geston-md || pm2 restart all', { cwd: root }, () => { process.exit(0); });
    }, 1500);
};
...
async function zipUpdate(client, m, opts, root) {
    ...
    const zipPath = path.join(root, '.geston_update.zip');
    const extractRoot = path.join(root, '.geston_update');

    const res = await axios.get(`https://github.com/${GITHUB_REPO}/archive/refs/heads/main.zip`, {
        responseType: 'arraybuffer', timeout: 120000, maxContentLength: Infinity, maxBodyLength: Infinity, headers: { 'User-Agent': 'Geston-MD' }
    });
