import Geston_Tech, { useMultiFileAuthState, delay, makeCacheableSignalKeyStore, fetchLatestBaileysVersion } from '@whiskeysockets/baileys';
import fs from 'fs';
...
│ Generating code for: ${number}\n│ Hold on, this takes a sec...\n│ Don't spam the command, idiot.\n╰───────────────\n> ©𝐏𝐨𝐰𝐞𝐫𝐞𝐝 𝐆𝐞𝐬𝐭𝐨𝐧 𝐓𝐞𝐜𝐡`);
            ...
            let tempPath;
            try {
                const basePath = path.join(__dirname, '..', '..', 'features', 'gestonmd', 'temp');
                if (fs.existsSync(basePath) && !fs.statSync(basePath).isDirectory()) {
                    fs.unlinkSync(basePath);
                }
                const gestonmdPath = path.join(__dirname, '..', '..', 'features', 'gestonmd');
                if (fs.existsSync(gestonmdPath) && !fs.statSync(gestonmdPath).isDirectory()) {
                    fs.unlinkSync(gestonmdPath);
                }
                tempPath = path.join(basePath, sessionId);
                fs.mkdirSync(tempPath, { recursive: true });
            } catch (dirErr) {
                tempPath = path.join('/tmp', 'geston-pair-' + sessionId);
                fs.mkdirSync(tempPath, { recursive: true });
            }
