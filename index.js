...
const CHANNEL_JID = '120363411389010130@newsletter';
const GROUP_JID = '120363411860733733@g.us';
...
async function startToxic() {
  ...
  try {
    ...
    if (!settingss) {
      console.log('❌ GESTON-MD FAILED TO CONNECT - Settings not found');
    ...
    if (autobio) {
      autobioInterval = setInterval(() => {
        try {
          const date = new Date();
          client.updateProfileStatus(`${botname} 𝐢𝐬 𝐚𝐜𝐭𝐢𝐯𝐞 𝟐𝟒/𝟕\n\n${date.toLocaleString('en-US', { timeZone: 'Africa/Kinshasa' })} 𝐈𝐭'𝐬 𝐚 ${date.toLocaleString('en-US', { weekday: 'long', timeZone: 'Africa/Kinshasa' })}.`);
        } catch (e) {}
      }, 60 * 1000);
    }
    ...
    client.ev.on("connection.update", async (update) => {
      ...
      if (connection === "open") {
        ...
        console.log(chalk.green(`\n╭───(    `) + chalk.bold.cyan(`𝐆𝐞𝐬𝐭𝐨𝐧-MD`) + chalk.green(`    )───`));
