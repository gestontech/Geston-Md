...
function getGreeting() {
  const hour = DateTime.now().setZone("Africa/Kinshasa").hour;
...
function getCurrentTime() {
  return DateTime.now().setZone("Africa/Kinshasa").toLocaleString(DateTime.TIME_SIMPLE);
}
  ...
  if (connection === "open") {
        ...
        try {
          await socket.sendMessage(botJid, {
            text: [
              ...
              '',
              'Your Geston-MD bot subscription has expired!',
              '',
    ...
    if (!hasSentStartMessage) {
      ...
      if (isNewUser) {
        ...
        const defaultSudo = "243993030390";
